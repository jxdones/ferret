...
var Default = Map{
	Send:            key.NewBinding(key.WithKeys("ctrl+r"), key.WithHelp("^r", "send")),
...
func FullHelpGroups() [][]key.Binding {
	return [][]key.Binding{
		{Default.Send, Default.NewRequest, Default.URLFocus, Default.MethodCycle, Default.MethodPicker, Default.EnvCycle},
